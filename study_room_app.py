...
EXAMS_DEFAULT = {
    "G検定": {
        "icon": "🤖",
        "description": "AIの基礎知識・ディープラーニングの理論",
        "color": "#6c63ff",
        "admin_url": ""   # 管理者が設定するデフォルトURL
    },
    "E資格": {
        "icon": "⚡",
        "description": "ディープラーニングのエンジニアリング実装",
        "color": "#e17055",
        "admin_url": ""
    },
    "AWS資格": {
        "icon": "☁️",
        "description": "AWSクラウドサービスの設計・運用",
        "color": "#fd9644",
        "admin_url": ""
    },
}
...
with st.sidebar:
    ...
    with st.expander("カスタム検定を追加"):
        ...
        if st.button("追加する", type="primary", use_container_width=True):
            if new_exam_name:
                st.session_state.custom_exams[new_exam_name] = {
                    "icon": new_exam_icon,
                    "description": new_exam_desc or "カスタム検定",
                    "color": "#a29bfe",
                    "admin_url": ""
                }
    ...
    with st.expander("デフォルトURLを設定（管理者用）"):
        ...
        for exam_name in all_exams:
            url_val = st.text_input(
                ...
                key=f"admin_url_{exam_name}"
            )
            st.session_state.admin_urls[exam_name] = url_val
        ...
        with st.container():
            ...
            <div class="exam-card active">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.8rem;">
                    <div>
                        <span style="font-size:1.5rem;">{exam['icon']}</span>
                        <strong style="font-size:1.1rem; margin-left:0.5rem;">{exam_name}</strong>
                    </div>
                    <span class="participants-badge">
                        👥 {participant_count}人が学習中
                    </span>
                </div>
            ...
            with cols[1]:
                if is_joined:
                    if st.button("退出する", key=f"leave_{exam_name}", type="secondary", use_container_width=True):
                        ...
                        st.success(f"「{exam_name}」ルームを退出しました")
                        st.rerun()
                else:
        ...
        with col:
            ...
            <div class="{card_class}">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:0.5rem;">
                    <div>
                        <span style="font-size:1.8rem;">{exam['icon']}</span>
                        <strong style="display:block; font-size:1.1rem; color:#1a1a2e; margin-top:0.2rem;">{exam_name}</strong>
                        <small style="color:#888;">{exam['description']}</small>
                    </div>
            ...
            if is_active and not is_joined:
                ...
                if st.button(f"🚀 ルームに参加する", key=f"join_{exam_name}", type="primary", use_container_width=True):
                    create_or_join_room(exam_name, room["url"], st.session_state.my_name)
                    st.success(f"「{exam_name}」のルームに参加しました！")
                    st.rerun()
                ...
                with st.expander(f"📢 今からやる！（ルームを作成）"):
                    ...
                    if admin_url and is_url_valid(admin_url):
                        st.markdown(f"<div class='alert-info'>管理者設定のURLを使用します</div>", unsafe_allow_html=True)
                        st.code(admin_url, language=None)
                        use_admin = st.checkbox("このURLを使う", value=True, key=f"use_admin_{exam_name}")
                        if use_admin:
                            url_to_use = admin_url
                        else:
                            url_to_use = st.text_input(
                                "別のURLを入力",
                                placeholder="https://meet.google.com/xxx-xxxx",
                                key=f"custom_url_{exam_name}"
                            )
                    else:
                        url_to_use = st.text_input(
                            "通話ルームURL",
                            placeholder="https://discord.gg/xxxxx または https://meet.google.com/xxx",
                            key=f"url_input_{exam_name}"
                        )
                        st.caption("Discord、Google Meet、Zoom など何でもOK！")

                    if st.button(
                        f"✅ ルームを作成して共有",
                        key=f"create_{exam_name}",
                        type="primary",
                        use_container_width=True
                    ):
                        final_url = url_to_use if isinstance(url_to_use, str) else ""
                        if not final_url or not is_url_valid(final_url):
                            st.error("有効なURL（http:// または https://）を入力してください")
                        else:
                            create_or_join_room(exam_name, final_url, st.session_state.my_name)
                            st.success(f"🎉 ルームを作成しました！")
                            st.balloons()
                            st.rerun()
